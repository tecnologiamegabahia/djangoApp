...
def uploads(request):
    ...
    try:
        ...
        return render(request, 'tipos_contribuyentes/upload.html')
...
def insert_value(values):
    try:
        ...
        sql = "INSERT INTO tipo_contribuyente_tipo_contribuyente(CODIGO, DESCRIPCION, ULTIMO_NODO, NIVEL, CODIGO_PADRE, BENEFICIOS_TRIBUTARIOS, created_at, updated_at, state)VALUES(%s, %s, %s, %s, %s, %s,current_timestamp(6), current_timestamp(6), '1');"
...
def drop_table():
    try:
        cursor = connection.cursor()
        sql = 'TRUNCATE table tipo_contribuyente_tipo_contribuyente'
        cursor.execute(sql)
    except Exception as e:
        print(e)
...
def insert_invalid(insert):
    archivo = open(settings.MEDIA_ROOT + "tipos_contribuyentes/registros-invalidos" + str(timestampStr) + ".txt", "w")
...
def insert_valid(insert):
    archivo = open(settings.MEDIA_ROOT + "tipos_contribuyentes/registros-validos" + str(timestampStr) + ".txt", "w")
    archivo.write(str(insert))
    archivo.close()
    return True
...
def verificarArchivos(request):
    with os.scandir(settings.MEDIA_ROOT + 'tipos_contribuyentes/') as ficheros:
        ficheros = [fichero.name for fichero in ficheros if fichero.is_file()]
        paginator = Paginator(ficheros, 5)
        page = request.GET.get('page')
        page_fichero = paginator.get_page(page)
        contexto = {'ficheros': page_fichero}
    return render(request, 'tipos_contribuyentes/lista_subidos.html', contexto)


def home(request):
    return render(request, 'tipos_contribuyentes/lista_subidos.html')


# def descargar archivos existentes
def download(request, path):
    print(path)
    file_path = os.path.join(settings.MEDIA_ROOT + 'tipos_contribuyentes/', path)
...
class DescargarArchivoView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        print(form)
        file_path = os.path.join(settings.MEDIA_ROOT + 'tipos_contribuyentes/', form)
        with open(file_path, 'rb') as fh:
