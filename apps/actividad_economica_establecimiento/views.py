...
def uploads(request):
    try:
        ...
        return render(request, 'actividad_economica_est/upload.html')
...
def insert_value(values):
    try:
        ...
        sql = "INSERT INTO actividad_economica_establecimiento_actividad_economica_est(ACTIVIDAD_ECONOMICA, ESTABLECIMIENTO, NUMERO_RUC, created_at, updated_at, state)VALUES(%s, %s, %s,current_timestamp(6), current_timestamp(6), '1');"
...
def insert_invalid(insert):
    archivo = open(settings.MEDIA_ROOT + "actividad_economica_est/registros-invalidos" + str(timestampStr) + ".txt",
                   "w")
...
def insert_valid(insert):
    archivo = open(settings.MEDIA_ROOT + "actividad_economica_est/registros-validos" + str(timestampStr) + ".txt", "w")
    archivo.write(str(insert))
    archivo.close()
    return True
...
def verificarArchivos(request):
    with os.scandir(settings.MEDIA_ROOT + 'actividad_economica_est/') as ficheros:
        ficheros = [fichero.name for fichero in ficheros if fichero.is_file()]
        paginator = Paginator(ficheros, 5)
        page = request.GET.get('page')
        page_fichero = paginator.get_page(page)
        contexto = {'ficheros': page_fichero}
    return render(request, 'actividad_economica_est/lista_subidos.html', contexto)


def home(request):
    return render(request, 'actividad_economica_est/lista_subidos.html')


# def descargar archivos existentes
def download(request, path):
    print(path)
    file_path = os.path.join(settings.MEDIA_ROOT + 'actividad_economica_est/', path)
...
class DescargarArchivoView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        print(form)
        file_path = os.path.join(settings.MEDIA_ROOT + 'actividad_economica_est/', form)
        with open(file_path, 'rb') as fh:
